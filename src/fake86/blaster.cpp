...
void cmdBlaster (uint8_t value) {
	...
	switch (value) {
			...
			case 0xF8: //undocumented command, clears in-buffer and inserts a null byte
				...
				break;
			default:
				log ("[NOTICE] Sound Blaster received unhandled command %02Xh\n", value);
				break;
		}
}
...
#ifdef DEBUG_BLASTER
	log ("[DEBUG] outBlaster: port %Xh, value %02X\n", portnum, value);
#endif
	portnum &= 0xF;
	switch (portnum) {
			case 0x0:
			...
			case 0x6: //reset port
				if ( (value == 0x00) && (blaster.lastresetval == 0x01) ) {
						...
						blaster.blockstep = 0;
						bufNewData (0xAA);
						setmem (mixer, 0xEE, sizeof (mixer) );
#ifdef DEBUG_BLASTER
...
void mixerReset() {
	setmem (blaster.mixer.reg, 0, sizeof (blaster.mixer.reg) );
