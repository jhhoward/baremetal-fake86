...
void createscalemap() {
	...
	xscale = (double) nw / (double) OUTPUT_DISPLAY_WIDTH;
	yscale = (double) nh / (double) OUTPUT_DISPLAY_HEIGHT;
	//if (scalemap != nullptr) free(scalemap);
	//scalemap = (void *)malloc( ((uint32_t)OUTPUT_DISPLAY_WIDTH + 1) * OUTPUT_DISPLAY_HEIGHT * 4);
	//if (scalemap == NULL) {
	//		printf("\nFATAL: Unable to allocate memory for scalemap!\n");
	//		exit(1);
	//	}
	scalemapptr = 0;
	for (dsty=0; dsty<(uint32_t)OUTPUT_DISPLAY_HEIGHT; dsty++) {
			srcy = (uint32_t) ( (double) dsty * yscale);
			scalemap[scalemapptr++] = srcy;
			for (dstx=0; dstx<(uint32_t)OUTPUT_DISPLAY_WIDTH; dstx++) {
